...
interface Props {
    ...
    /** Only use in tests. There's a bug with onboarding swiper and Flatlist that hinders testing page 1 and 2 integratively */
    onlyRenderPage?: number;
}

const IOSOnboardingScreen: FC<Props> = ({
    finishOnboarding,
    onlyRenderPage,
}) => {
    ...
    const nothing = <></>;
    const pages: Onboarding["props"]["pages"] = [
        {
            ...
            subtitle: (
                <>
                    <View style={styles.pushNotificationContainer}>
                        ...
                        <Paragraph style={styles.pushNotificationSideNote}>
    ...
    const usedPages = !isNil(onlyRenderPage)
        ? pages.filter((_, i) => i === onlyRenderPage)
        : pages;
    return (
        <Onboarding
...
const mapDispatchToProps: Pick<Props, "finishOnboarding"> = {
